...
/**
 * Seeds the GLOBAL catalogue: features, permissions, the six system roles and
 * their grants. No tenant data.
 *
 * Lives in src/ rather than in the script so the tests can call it too: a test
 * that truncates enterprises also cascades into the NULL-enterprise role
 * templates, and signup then fails with "the owner template is missing" — which
 * is correct behaviour, so the fix is to re-seed rather than to weaken it.
 *
 * Idempotent: every insert names the real unique index (three of the four are
 * partial, so the inference clause repeats the predicate) and conflicts DO
 * NOTHING.
 */
export async function seedCatalogue(manager: EntityManager): Promise<SeedReport> {
  assertScopesAgree();
